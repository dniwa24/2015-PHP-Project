  ...
  <body>
	...
	<table class="table table-bordered">
		...
		<tbody>
			<?php
				// NEW CODE
				$sql = "SELECT * FROM users INNER JOIN userlv ON users.userlevel = userlv.userlevel";
				$result = $con->query($sql) or die(mysqli_error());
				while($row = $result->fetch_array(MYSQLI_ASSOC)){

				// OLD CODE
				// $viewquery = mysql_query("SELECT * FROM users INNER JOIN userlv ON users.userlevel = userlv.userlevel");
			...
			<tr>
				...
				<td><a class="navbar-brand" href="update.php?id=<?php echo $row['userid'] ?>">Update</a>
					<a class="navbar-brand"	href="Delete.php?id=<?php echo $row['userid'] ?>">Delete</a></td>
			</tr>
		</tbody>
		<?php } //mysql_close($con); ?>
	</table>
